...
1. `file_dispute` locks the claimant’s GEN with a claim and evidence URLs
2. `submit_defense` requires the respondent to **match the same stake** before judgment
3. `judge_dispute` scrapes both evidence sets via AI consensus and settles the escrow
...
|----------|------|-------------|
| `file_dispute` | write (payable) | Opens a case and locks claimant GEN with evidence URLs |
